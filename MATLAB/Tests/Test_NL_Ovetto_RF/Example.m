...
N_trunc = 5 ; % number of KL bases

[F,KL] = randomfield(corr,MESH.nodes','trunc', N_trunc , 'mean' , 2.39);

% Get a realization and plot it 
NS = 1 ; % number of samples

sample = randn(N_trunc,NS) ;
...
%% smooth random fields coefficients! 

F_RF_smoothed = F_RF + (DATA.Mi + DATA.Me - F_RF).*( 1 - smoothLS( DATA.heartLS(MESH.nodes(1,:)',MESH.nodes(2,:)') , 0.2) ) ;

% if (PLOT_ALL)
   figure
   pdeplot(MESH.vertices,[],MESH.elements(1:3,:),'xydata',F_RF_smoothed(1:MESH.numVertices),'xystyle','interp',...
        'zdata',F_RF_smoothed(1:MESH.numVertices),'zstyle','continuous',...
        'colorbar','on', 'mesh' , 'off'  );
   colormap(jet); lighting phong; title('Realization of the random field')
   view([0 90])
   axis equal
...
% Parte sul torso che dipende dal RF e sul cuore (dopo smoothing)
% Example call : A = Assembler_2D_RF(MESH, DATA, FE_SPACE , 'diffusion' , [] , [] , FLAG_HEART_REGION, [] , F )
A_rf = NonLinear_Assembler_2D_RF_smoothed( MESH , DATA_Zd , FE_SPACE  , 'diffusion' , [] , [] , [] , [] , F_RF_smoothed ) ;
...
zd = solveFwdNL( MESH , FE_SPACE , DATA_Zd , w , A_rf + A_wdep , B  ) ; 
t_solve = toc(t_solve);
fprintf('done in %3.3f s \n', t_solve);
...
%% Lancia un solver deterministico per cercare di trovare l'ischemia
...
for i=1:2000
    ...
    while( ~ACCEPTABLE) 
        ...
        t_assembly = tic;
        A_wdep              =  NonLinear_Assembler_2D(MESH, DATA, FE_SPACE , 'diffusion' , ...
            [] , [] , DATA.FLAG_HEART_REGION ,[] ,w_new_projected );
        t_assembly = toc(t_assembly);
        fprintf('done in %3.3f s', t_assembly);
...
%% Lanciamo ora un solver che tenga conto della parte stocastica

% Prima di tutto creiamo una sparse grid
...
S = smolyak_grid( N_trunc ,w,knots,@lev2knots_doubling);
